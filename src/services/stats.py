...
from src.database.models import GlobalStats, CoinStats
    ...
    async def format_global_stats_message(self) -> str:
        ...
        if stats.top_coins:
            ...
            for i, (symbol, rate, count) in enumerate(stats.top_coins[:5]):
                ...
                lines.append(f"{medals[i]} {coin_name} - <b>{rate:.0f}%</b> ({count} pumps)")
            lines.append("")
        ...
        if stats.today_pumps > 0:
            lines.append(
                f"Pumps: <b>{stats.today_pumps}</b> | "
                f"Wins: <b>{stats.today_hit_50pct}</b> | "
                f"Rate: <b>{stats.today_success_rate:.0f}%</b>"
            )
        else:
